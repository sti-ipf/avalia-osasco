    ...
    Prawn::Document.generate("#{RAILS_ROOT}/public/relatorios/final/#{ue.id}_#{service_level.id}.pdf", :margin => [30, 30, 30, 30]) do
      ...
      def number_pages(string, position)
        page_count.times do |i|
          go_to_page(i+1)
          str = string.gsub("<page>","#{i+4}").gsub("<total>","#{page_count}")
      ...
      def default_table(data, options={})
        options = {
          :header => ["Segmento", "Nº da questão do segmento", " média por Segmento", "média da UE"]
        }.merge(options)
      ...
      def draw_indicator_tables(indicator, questions_party, service_level)
        ...
        segments = Rails.cache.fetch("report-segments") { service_level.segments }.sort
      ...
      def question_table(ue,question_party)
        ...
        data = @report_data.questions_party_table_burjato(question_party)
        questions_party_table = data[:table]

        start_new_page if y < 400

        text "\n #{data[:description]}"
        table questions_party_table, :column_widths => { 0 => 4 } do
          column(0).width = 90

          cells[1,0].borders = [:left, :bottom, :right, :top]
          cells[1,1].borders = [:left, :bottom, :right, :top]
          cells[1,2].borders = [:left, :bottom, :right, :top]
          cells[1,3].borders = [:left, :right]

          cells[2,0].borders = [:left, :bottom, :right, :top]
          cells[2,1].borders = [:left, :bottom, :right, :top]
          cells[2,2].borders = [:left, :bottom, :right, :top]
          cells[2,3].borders = [:left, :right]

          cells[3,0].borders = [:left, :bottom, :right, :top]
          cells[3,1].borders = [:left, :bottom, :right, :top]
          cells[3,2].borders = [:left, :bottom, :right, :top]
          cells[3,3].borders = [:left, :bottom, :right]
...
# inicio do texto
  ...
  text "#{ue.name}", :align => :center, :size => 15, :style => :bold
  ...
  text "Neste gráfico podemos analisar os resultados das médias da dimensão por segmento pesquisado.
    As colunas referentes a cada segmento apresentarão a média das respostas atribuídas pela unidade
    escolar à dimensão analisada (exemplo: média das respostas dos familiares da unidade).", :indent_paragraphs => 40


  text "\n Percepção da UE sobre cada indicador de qualidade", :style => :bold
  text "Gráficos que apresentam as médias de cada indicador de qualidade por segmento. A organização dos gráficos em colunas de cada segmento é a mesma dos gráficos das dimensões.", :indent_paragraphs => 40
  text "Cabe destacar que em virtude de alguns indicadores de qualidade terem sido suprimidos para alguns segmentos, a numeração dos mesmos não segue a mesma sequência para todos.", :indent_paragraphs => 40

  text "\n Médias das respostas atribuídas a cada questão que compõe os indicadodres da dimensão, por segmento escolar", :style => :bold
  text "Quadro que apresenta a média das opiniões numéricas atribuídas a cada questão pelos segmentos escolares participantes da avaliação. É importante ressaltar que a quantidade de questões formuladas a cada segmento foi diferente. Por essa razão, os quadros das questões serão apresentados tomando como referência o instrumental respondido pelos gestores, por ser o mais abrangente.", :indent_paragraphs => 40
  ...
  text "\n 2.1.3 Questões problematizadoras da Dimensão 1", :style => :bold
  ...
  text " • Que ações podem ser previstas em 2011 para melhorar as relações na acolhida das pessoas? Quais segmentos estarão envolvidos em cada ação de melhorias do ambiente educativo?"
  ...
  text "\n 2.2.3 Questões problematizadoras da dimensão 2:", :style => :bold
  ...
  text "• Como estamos organizando nosso espaço físico e os materiais para que estes incentivem e motivem a expressão dos educandos?"
  ...
  text "\n 2.3.3 Questões problematizadoras da dimensão 3:", :style => :bold
  text "• O que pode ser pensado em nosso PTA 2011 para aumentar o desejo de aprender dos educandos? O que pode ser previsto em nosso PTA2011 para que os educandos ampliem suas oportunidades de desenvolverem autonomia na busca por conhecimentos?"
  ...
  text "\n 2.4.3 Questões problematizadoras da dimensão 4", :style => :bold
  ...
  text "\n 2.5.3 Questões problematizadoras da dimensão 5", :style => :bold
  ...
  text "\n 2.6.3 Questões problematizadoras da dimensão 6", :style => :bold
  ...
  text "\n 2.7.3 Questões problematizadoras da dimensão 7", :style => :bold
  ...
  text "\n 2.8.3 Questões problematizadoras da dimensão 8", :style => :bold
  ...
  text "\n 2.9.3 Questões problematizadoras da dimensão 9", :style => :bold
  ...
  text "• A unidade cria um ambiente e uma atmosfera adequada à participação?"
  ...
  text "\n 2.10.3 Questões problematizadoras da dimensão 10", :style => :bold
  ...
  text "\n 2.11.3 Questões problematizadoras da dimensão 11", :style => :bold
  text "• Que ações podem ser previstas para que em 2011 outros segmentos da escola se envolvam no exercício da função social da escrita? "
  text "• Que propostas podemos planejar para que em 2011 haja atividades lúdicas nas quais o educando exercite a expressão de suas produções e registros (abordando diversas esferas de circulação, práticas de linguagem, contextos e gêneros)? Que propostas podem ser previstas para que o educando socialize com diferentes segmentos as suas produções e registros?"
  ...
  text "\n\n 3 QUADRO DE ÍNDICES DA SUA UNIDADE EDUCACIONAL", :style => :bold
  fill_color "000000"
  ...
  index_data = report_data.index_table_burjato
  text "O índice geral da sua Unidade em 2010, obtido com base na média dos índices de cada dimensão é: #{index_data[:institution_main_index]}"
  table index_data[:table] do
